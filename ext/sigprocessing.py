# Adapted from python_speech_features | spoken-language-identification
#
...
def deframesig(frames, siglen, frame_len, frame_step,
    ...
    indices = numpy.tile(numpy.arange(0, frame_len), (numframes, 1)) + numpy.\
        tile(numpy.arange(0, numframes * frame_step, frame_step),
                         (frame_len, 1)).T
    indices = numpy.array(indices, dtype=numpy.int32)
...
def magspec(frames, nftt):
    ...
    :returns: If frames is an NxD matrix, output will be Nx(NFFT/2+1).
              ...
              frame.
    """
    if numpy.shape(frames)[1] > nftt:
        warnings.warn('frame length (%d) is greater than FFT size (%d), '
                      'frame will be truncated. Increase NFFT to avoid.',
                      numpy.shape(frames)[1], nftt)

    complex_spec = numpy.fft.rfft(frames, nftt)
    return numpy.absolute(complex_spec)


def powspec(frames, nftt):
    """
    Compute the power spectrum of each frame in frames.
    If frames is an NxD matrix, output will be Nx(NFFT/2+1).

    :param frames: the array of frames. Each row is a frame.
    :param nftt: the FFT length to use. If NFFT > frame_len,
                 the frames are zero-padded.
    :returns: If frames is an NxD matrix, output will be Nx(NFFT/2+1).
              Each row will be the power spectrum of the corresponding frame.
    """
    return 1.0 / nftt * numpy.square(magspec(frames, nftt))


def logpowspec(frames, nftt, norm=1):
    """
    ...
    :param nftt: the FFT length to use. If NFFT > frame_len, 
                 the frames are zero-padded.
    :param norm: If norm=1, the log power spectrum is normalised so that
                 the max value (across all frames) is 0.
    :returns: If frames is an NxD matrix, output will be Nx(NFFT/2+1).
              Each row will be the log power spectrum of the corresponding
              frame.
    """
    ps = powspec(frames, nftt)
    ps[ps <= 1e-30] = 1e-30
