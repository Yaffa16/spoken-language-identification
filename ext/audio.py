...
#
# calculate filterbank features. Provides e.g. fbank and mfcc features
# for use in ASR applications
...
def mfcc(signal, samplerate=16000, winlen=0.025, winstep=0.01, numcep=13,
         ...
         ceplifter=22, append_energy=True,
    ...
    if append_energy: feat[:, 0] = numpy.log(
...
def fbank(signal, samplerate=16000, winlen=0.025, winstep=0.01,
    ...
    :param signal: the audio signal from which to compute features. Should be
                   an N*1 array
    :param samplerate: the samplerate of the signal we are working with.
    :param winlen: the length of the analysis window in seconds. Default is
                   0.025s (25 milliseconds)
    ...
    :param winfunc: the analysis window to apply to each frame. By default no
                    window is applied. You can use numpy window functions here
    ...
    feat = numpy.where(feat == 0, numpy.finfo(float).eps,
...
def ssc(signal, samplerate=16000, winlen=0.025, winstep=0.01,
    ...
    frames = sigproc.framesig(signal, winlen * samplerate, winstep * samplerate,
                              winfunc)
    pspec = sigproc.powspec(frames, nfft)
    pspec = numpy.where(pspec == 0, numpy.finfo(float).eps,
...
def delta(feat, n):
    ...
    :param feat: A numpy array of size (num_frames by number of features)
                 containing features. Each row holds 1 feature vector.
    :param n: For each frame, calculate delta features based on preceding and
              following N frames
    :returns: A numpy array of size (num_frames by number of features)
    ...
    for t in range(num_frames):
        delta_feat[t] = numpy.dot(numpy.arange(-n, n + 1), padded[
                                                           t: t + 2 * n + 1]) \
